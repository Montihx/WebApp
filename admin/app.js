(() => {
  ...
  function renderControlCenter() {
    ...
    const jobRows = jobsSample
      .map((job) => {
        const progress =
          job.progress === null
            ? `<span class="honest-progress"><i></i>Нет точного прогресса</span>`
    ...
    const jobStatusStrip = `<div class="status-strip" role="img" aria-label="Статусы среди ${jobsSample.length} загруженных задач: ${jobStatusLegend}">${jobStatusOrder
      .map(
        (key) =>
          `<span class="status-strip__segment status-strip__segment--${jobStatusTone[key]}" style="width:${(jobStatusCounts[key] / jobsSample.length) * 100}%" data-tooltip="${jobStatusLabel[key]} · ${jobStatusCounts[key]} из ${jobsSample.length}"></span>`,
      )
      .join("")}</div><div class="queue-legend">${jobStatusOrder
      .map(
        (key) =>
          `<span class="queue-legend__item"><i class="queue-legend__dot status-strip__segment--${jobStatusTone[key]}"></i>${jobStatusLabel[key]}<b>${jobStatusCounts[key]}</b></span>`,
      )
      .join("")}</div>`;
    const processedDays = [
      ["9 авг", 742, 210],
    ...
    const trendColumns = processedDays
      ...
      .join("");
    const processedChart = `<section class="panel panel--flush trend-panel"><header class="panel-header"><div class="panel-title-group"><span class="section-kicker">Обработка источников</span><h2 class="panel-title">Обработано за 7 дней</h2><p class="panel-subtitle">Записи из Kodik и Shikimori · 9–15 августа</p></div><div class="chart-legend"><span class="legend-item"><i class="legend-mark"></i>Kodik · ${totalKodik.toLocaleString("ru-RU")}</span><span class="legend-item"><i class="legend-mark legend-mark--info"></i>Shikimori · ${totalShikimori.toLocaleString("ru-RU")}</span><span class="chart-summary">За неделю <strong>${(totalKodik + totalShikimori).toLocaleString("ru-RU")}</strong></span></div></header><div class="trend-grid" role="img" aria-label="Обработано за 7 дней: Kodik и Shikimori по дням">${trendColumns}</div></section>`;
    return `${heading({ eyebrow: "Демонстрационные данные", title: "Обзор", description: "Задачи, требующие внимания, и состояние сервисов.", actions: `${actionButton("Найти в источниках", "go-imports", "search-code")}${actionButton("Ручной запуск", "start-sync", "play", true)}` })}<div class="metric-grid"><article class="metric-card metric-card--focus"><span>Пользователи</span><strong>24 812</strong><small>1 284 активны за сутки</small><div class="metric-ratio" role="img" aria-label="1 284 из 24 812 активны за сутки"><span style="width:${((1284 / 24812) * 100).toFixed(1)}%"></span></div></article><article class="metric-card"><span>Аниме</span><strong>3 402</strong><small>184 онгоинга</small><div class="metric-ratio" role="img" aria-label="184 из 3 402 в статусе онгоинг"><span style="width:${((184 / 3402) * 100).toFixed(1)}%"></span></div></article><article class="metric-card"><span>Активные релизы</span><strong>18 429</strong><small>доступны в плеере</small></article><article class="metric-card"><span>Время работы API</span><strong>3д 11ч</strong><small>с последнего запуска</small></article></div><div class="control-layout">${processedChart}<div class="control-aside"><section class="panel panel--flush decision-panel"><header class="panel-header"><div class="panel-title-group"><span class="section-kicker">Очередь</span><h2 class="panel-title">Требуется оператор</h2><p class="panel-subtitle">Проверьте ошибки, конфликты и материалы на модерации.</p></div><span class="queue-total">20 открыто</span></header>${queueBreakdown}<div class="decision-list">${decisionRows}</div></section><aside class="active-operation"><div class="active-operation__head"><div><span class="section-kicker">Выполняется</span><h2>Kodik full sync</h2><p>#J-8412 · ручной запуск</p></div>${status(labelForStatus("running"), toneForStatus("running"))}</div><div class="operation-clock"><strong>18:42</strong><span>прошло с запуска</span></div><div class="indeterminate-track" role="progressbar" aria-label="Задача выполняется без публикации точного прогресса"><span></span></div><div class="truth-note">${icon("info")}<p><strong>Точный прогресс недоступен.</strong><br>Результат появится в журнале после завершения задачи.</p></div><div class="operation-actions"><button class="button button--primary" data-action="view-job" data-id="#J-8412">${icon("panel-right-open")}Открыть задачу</button><button class="button button--danger" data-action="stop-job">${icon("square")}Остановить</button></div></aside></div></div><section class="health-ribbon" aria-label="Состояние сервисов"><button data-view="monitoring"><span>Go API</span><strong><i class="status-dot status-dot--ok"></i>online</strong><small>последняя проверка</small></button><button data-view="monitoring"><span>PostgreSQL</span><strong><i class="status-dot status-dot--ok"></i>12 ms</strong><small>основная база</small></button><button data-view="monitoring"><span>Cache</span><strong><i class="status-dot status-dot--warning"></i>не проверен</strong><small>проверка доступности не выполнена</small></button><button data-view="parsers"><span>Workers</span><strong><i class="status-dot status-dot--ok"></i>2 активны</strong><small>работающие процессы</small></button><button data-view="monitoring"><span>Storage</span><strong><i class="status-dot status-dot--warning"></i>68%</strong><small>занято на диске</small></button></section><section class="panel panel--flush"><header class="panel-header"><div class="panel-title-group"><span class="section-kicker">Журнал</span><h2 class="panel-title">Последние задачи</h2><p class="panel-subtitle">Последние запуски и результаты обработки.</p></div><button class="panel-link" data-view="parsers">Задачи и логи ${icon("arrow-right")}</button></header>${jobStatusStrip}<div class="table-shell"><table class="data-table responsive"><thead><tr><th>Задача</th><th>Статус</th><th>Прогресс</th><th>Старт</th><th>Длительность</th><th></th></tr></thead><tbody>${jobRows}</tbody></table></div></section>`;
  }
  ...
  function renderParserCenterReal() {
    ...
    return `${heading({ eyebrow: "Потоки данных · parser jobs", title: "Парсер-центр", description: "Поддерживаемые режимы, честный прогресс, результаты и REST-журнал в одном месте.", actions: `${actionButton("Обновить", "refresh-jobs", "refresh-cw")}${actionButton("Ручной запуск", "start-sync", "play", true)}` })}<div class="section-bar parser-tabs">${sectionTabs(
      ...
      state.parserTab,
      "parser-tab",
    )}<div class="section-bar__meta"><span class="status-dot status-dot--ok"></span>2 активны · count из Asynq Inspector</div></div>${body}`;
  }
