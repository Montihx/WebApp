...
Изменения после этой даты (accent-палитра, честные визуализации на Обзоре — decision queue breakdown, job status strip, 7-day processed chart как block-grid bar с hover-tooltip; экспорт в Коллекциях и Настройках парсера; клиентский фильтр по роли в Пользователях; favicon) переверены отдельными Playwright-прогонами headless Chromium по всем 17 маршрутам на `1440×900` и мобильных ширинах — `0` console errors, `0` duplicate IDs, `0` горизонтального overflow. Полный ручной 184-interaction чек-лист ниже не переигрывался заново под новые элементы; актуальное машинное состояние — `admin/qa-results.json` (`18/18 pass`).
