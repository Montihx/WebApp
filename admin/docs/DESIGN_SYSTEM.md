...
- Не пиши общий селектор вида `.контейнер span`/`.контейнер > *` для стилизации подписи, если внутри контейнера может оказаться `.status-badge`, `.job-glyph`, `.event-icon` или другой тон-компонент. Либо исключай его через `:not(.status-badge)`, либо адресуй подпись явным классом.
