...
- Под названием расположены три быстрых элемента: собственный статус списка, `favorites_count` и `comments_count`; следом — одно главное действие просмотра. На desktop эти же подтверждённые счётчики входят в statistics. Факты остаются вертикальными строками «иконка — подпись — значение», а разделители начинаются после иконки и намеренно почти растворяются в фоне.
