...
- Под названием расположены три быстрых элемента в пропорциях референса: собственный статус без лишней ведущей иконки, видимый `favorites_count` с bookmark и отдельная компактная кнопка комментариев. Её accessible name содержит `comments_count`; на desktop/tablet оба подтверждённых числа входят в statistics. Факты оформлены как цельные естественные строки «иконка — готовый текст» без label/value-колонок, карточек и разделителей.
- На desktop настройка уведомлений относится к сериям и остаётся в toolbar списка серий. На mobile она занимает правую кнопку поверх постера вместо повторной закладки и открывает bottom sheet. Отдельное share-действие и повторная секция подробностей не показываются.
