...
- Сохранить API-driven пять слайдов production `Hero`, интервал `9000 ms` и единственную нижнюю полосу прогресса с тем же интервалом. По последнему заданию полностью удалить pause/play-кнопку, а на телефоне (`≤720 px`) скрыть стрелки; desktop-стрелки, keyboard, touch swipe и reduced-motion сохранить. После касания, фокуса или ручного переключения автопрокрутка не возобновляется до перезагрузки. Плашку пагинации, счётчик и «О тайтле» не возвращать; fixture-разметку слайдов в компонент не переносить.
