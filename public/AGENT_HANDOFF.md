...
- Сохранить API-driven пять слайдов production `Hero`; по последнему заданию заменить прежний интервал `7000 ms` на `9000 ms`, добавить единственную нижнюю полосу прогресса с тем же интервалом и отдельную pause/play-кнопку рядом со стрелками. Сохранить keyboard, touch swipe и reduced-motion. Плашку пагинации, счётчик и «О тайтле» не возвращать; fixture-разметку слайдов в компонент не переносить.
